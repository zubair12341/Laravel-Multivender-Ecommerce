...
class ProductsController extends Controller
{
    ...
    public function listing(Request $request)
    {
        ...
        if (isset($_GET['brand']) && !empty($_GET['brand'])) {
            $categoryProducts->where('brand_id', $_GET['brand']);
        }
        if (isset($_GET['category']) && !empty($_GET['category'])) {
            $categoryProducts->where('category_id', $_GET['category']);
        }
        if (isset($_GET['product_search']) && !empty($_GET['product_search'])) {
            $categoryProducts->where('product_name', 'like', '%' . $_GET['product_search'] . '%');
        }
        if (isset($_GET['section_id']) && !empty($_GET['section_id'])) {
            $categoryProducts->where('section_id', $_GET['section_id']);
        }

        if (isset($_GET['from_price']) && !empty($_GET['from_price'])) {
            $categoryProducts->where('product_price', '>=', $_GET['from_price']);
        }

        if (isset($_GET['to_price']) && !empty($_GET['to_price'])) {
            $categoryProducts->where('product_price', '<=', $_GET['to_price']);
        }
        ...
        return view('front.products.listing')->with(compact('categoryDetails', 'categoryProducts', 'sections', 'brands'));
    }
    ...
    public function cartUpdate(Request $request)
    {
            ...
            if ($data['qty'] > $availableStock['stock']) { // if the user's desired quantity exceeds the available `stack` in `products_attributes` table    // $data['cartid'] comes from the 'data' object sent from inside the $.ajax() method in front/js/custom.js file
                ...
                return redirect()->back()->with('success_message', 'Product Stock is not available');
            }
            ...
            if ($availableSize == 0) { // if the desired product's `status` in `products_attributes` table is zero 0 (inactive/disabled)
                ...
                return redirect()->back()->with('success_message', 'Product Size is not available. Please remove this Product and choose another one!');
            }
            ...
            return redirect()->back()->with('success_message', 'Cart has been updated');
    ...
    public function applyCoupon(Request $request)
    {
            ...
            if ($couponCount == 0) { // if the submitted coupon is wrong, send error message
                ...
                return redirect()->back()->with('error_message', 'The coupon is invalid!');
    ...
    public function checkout(Request $request)
    {
        ...
        if ($request->isMethod('post')) { // if the <form> in front/products/checkout.blade.php is submitted (the HTML Form that the user submits to submit their Delivery Address and Payment Method)
            ...
            } elseif($data['payment_gateway'] == 'Stripe') { // if the `payment_gateway` selected by the user is not 'COD', meaning it's like PayPal, Prepaid, ... (in front/products/checkout.blade.php), we send the placing the order confirmation email and SMS after the user makes the payment
                return redirect()->route('online-pay',$order->id);
            }
