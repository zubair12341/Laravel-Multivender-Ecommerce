    ...
    public function dashboard() {
        ...
        if(Auth::guard('admin')->user()->type == 'vendor')
        {
            ...
            $ordersCount = Order::with([
                // Eager loading the 'orders_products' relationship
                'orders_products' => function($query) use ($vendor_id) {
                    // Applying a constraint to only include products with the specified vendor_id
                    $query->where('vendor_id', $vendor_id);
                }
            ])->whereHas('orders_products', function($query) use ($vendor_id) {
                // Adding a condition to the main query to ensure only orders with products from the specified vendor are counted
                $query->where('vendor_id', $vendor_id);
            })->count();
            $productsCount   = Product::where('vendor_id',$vendor_id)->count();
            $couponsCount    = Coupon::where('vendor_id',$vendor_id)->count();
        }
        else
        {
        ...
        if ($request->isMethod('post')) { // if the update <form> is submitted
            ...
            if ($request->hasFile('admin_image')) { // the HTML name attribute    name="admin_name"    in update_admin_details.blade.php
                ...
                if ($image_tmp->isValid()) {
                    ...
                    // Save the image to the specified path in storage/app/public/front/images/banner_images
                    $image_tmp->storeAs('public/admin/images/photos/', $imageName);
            ...
            if ($request->isMethod('post')) { // if the <form> is submitted
                ...
                if ($request->hasFile('address_proof_image')) { // the HTML name attribute    name="admin_name"    in update_admin_details.blade.php
                    ...
                    if ($image_tmp->isValid()) {
                        ...
                        // Save the image to the specified path in storage/app/public/front/images/banner_images
                        $image_tmp->storeAs('public/admin/images/proofs/', $imageName);
                    }
