    ...
    public function addEditProduct(Request $request, $id = null) { // If the $id is not passed, this means 'Add a Product', if not, this means 'Edit the Product'    
        ...
        if ($request->isMethod('post')) { // WHETHER 'Add a Product' or 'Update a Product' <form> is submitted (THE SAME <form>)!!
            ...
            if ($request->hasFile('product_image')) {
                $image_tmp = $request->file('product_image'); // Retrieving Uploaded Files: https://laravel.com/docs/9.x/requests#retrieving-uploaded-files
                ...
                if ($image_tmp->isValid()) {
                    ...
                    $image_tmp->storeAs('public/front/images/product_images/large/', $imageName);
                    $image_tmp->storeAs('public/front/images/product_images/medium/', $imageName);
                    $image_tmp->storeAs('public/front/images/product_images/small/', $imageName);
